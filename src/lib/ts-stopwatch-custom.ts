...
export class Stopwatch {
	 ...
	 */
	public startSystemTime: number | undefined;
	 ...
	 */
	public stopSystemTime: number | undefined;
	...
	public setTime(newTime: number): void {
		if (typeof newTime !== 'number' || !isFinite(newTime) || newTime < 0) {
			throw new Error('setTime expects a non-negative finite number');
		}

		// If never started (IDLE) — initialize startSystemTime at a logical system time
		// so that calculateStopwatchTime() can compute end - start - stopDuration = newTime.
		if (this.startSystemTime === undefined) {
			// Use current system time as baseline and move pending slice start to 0
			const systemNow = this.getSystemTime();
			this.startSystemTime = systemNow - newTime;
			this.stopSystemTime = undefined;
			this.stopDuration = 0;
			this.pendingSliceStartStopwatchTime = 0;
			return;
		...
		// Get a system-time equivalent that corresponds to the current stored stopwatch time
		const currentSystemTime = this.getSystemTimeOfCurrentStopwatchTime();
		const currentStopwatchTime = this.calculateStopwatchTime(currentSystemTime);

		// If no change, nothing to do
		if (currentStopwatchTime === newTime) {
			return;
		}

		// We will adjust startSystemTime so that:
		// newTime === currentSystemTime - newStartSystemTime - stopDuration
		// => newStartSystemTime = currentSystemTime - newTime - stopDuration
		const newStartSystemTime = currentSystemTime - newTime - this.stopDuration;

		// Update startSystemTime
		this.startSystemTime = newStartSystemTime;

		// If stopwatch is stopped, ensure stopSystemTime still points to a valid system time
		// representing the same stopwatch time as before (but expressed against new startSystemTime).
		if (this.stopSystemTime !== undefined) {
			// Keep the stopwatch stopped at the same system instant (no change needed),
			// but ensure invariants are consistent — stopSystemTime must remain unchanged.
			// (Because getSystemTimeOfCurrentStopwatchTime() returned stopSystemTime earlier.)
		}

		// Adjust pendingSliceStartStopwatchTime: it is measured in stopwatch time.
		// If pendingSliceStartStopwatchTime is defined, keep same offset relative to stopwatch time.
		// No change needed because pendingSliceStartStopwatchTime is expressed in stopwatch time
		// and setTime changes the mapping between system time and stopwatch time but not the
		// stopwatch-time origin of slices. However, if the start was previously undefined and
		// we setTime from IDLE above, it's already handled.
		// If pendingSliceStartStopwatchTime is defined but greater than newTime (shouldn't normally),
		// clamp it to newTime to preserve valid slice durations.
		if (
			this.pendingSliceStartStopwatchTime !== undefined &&
			this.pendingSliceStartStopwatchTime > newTime
		) {
			this.pendingSliceStartStopwatchTime = newTime;
		}
	}
...
export namespace Stopwatch {
	 ...
	 *        Defaults to {@link Date.now}.
	 */
	export function setDefaultSystemTimeGetter(systemTimeGetter: GetTimeFunc = Date.now): void {
